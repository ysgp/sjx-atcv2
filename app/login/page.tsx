...
function LoginContent() {
  ...
  const [isLoading, setIsLoading] = useState<'discord' | 'vamsys' | null>(null);
  ...
  const handleVamsysLogin = () => {
    // vAMSYS OAuth 尚在開發中
    setIsLoading('vamsys');
    // TODO: 實作 vAMSYS OAuth
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card border border-cream/20">
          <div className="text-center mb-6">
            ...
            <p className="text-cream/60">請選擇登入方式以繼續</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {/* vAMSYS Login */}
            <button
              onClick={handleVamsysLogin}
              disabled={true}
              className="w-full px-6 py-4 bg-cream/10 hover:bg-cream/20 disabled:opacity-50 disabled:cursor-not-allowed text-cream rounded-xl font-semibold transition-all flex items-center justify-center gap-3 border border-cream/20 relative"
            >
              {isLoading === 'vamsys' ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : (
              ...
              <span>使用 vAMSYS 登入</span>
              <span className="absolute top-2 right-2 text-[10px] bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-full">
                開發中
              </span>
            </button>
          </div>
